...
proc ExportStressForSet {} {
    ...
    set c_shear "MaxShear"  ;# NEW: MaxShear component
    ...
    foreach sub $subcase_list {
        ...
        # Format for CSV
        set str_von [expr {$max_von != "" ? "[format \"%.2f\" $max_von]" : "N/A"}]
        set str_p1 [expr {$max_p1 != "" ? "[format \"%.2f\" $max_p1]" : "N/A"}]
        set str_p3 [expr {$min_p3 != "" ? "[format \"%.2f\" $min_p3]" : "N/A"}]
        set str_shear [expr {$max_shear != "" ? "[format \"%.2f\" $max_shear]" : "N/A"}]
    ...
    # Global extremes row
    puts $fp "---,---,---,---,---,---,---,---,---,---"
    set g_str_von [expr {$g_max_von != "" ? "[format \"%.2f\" $g_max_von]" : "N/A"}]
    set g_rf_von [SafeRF $FtuStress $g_max_von]
    set g_str_p1 [expr {$g_max_p1 != "" ? "[format \"%.2f\" $g_max_p1]" : "N/A"}]
    set g_rf_p1 [SafeRF $FtuStress $g_max_p1]
    set g_str_p3 [expr {$g_min_p3 != "" ? "[format \"%.2f\" $g_min_p3]" : "N/A"}]
    set g_rf_p3 [SafeRF $FtuStress $g_min_p3]
    set g_str_shear [expr {$g_max_shear != "" ? "[format \"%.2f\" $g_max_shear]" : "N/A"}]
    set g_rf_shear [SafeRF $FsuStress $g_max_shear]
    
    puts $fp "GLOBAL_EXTREMES,ALL_SUBCASES,$g_str_von,$g_rf_von,$g_str_p1,$g_rf_p1,$g_str_p3,$g_rf_p3,$g_str_shear,$g_rf_shear"
