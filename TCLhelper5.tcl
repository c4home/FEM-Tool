...
button .exportUI.btn -text "Export Displacements (all)" -command ExportDisplacement
...
frame .exportUI.frmAllowStress
pack .exportUI.frmAllowStress -pady 5
label .exportUI.frmAllowStress.lbl -text "Ftu" 
entry .exportUI.frmAllowStress.entFtu -textvariable ::FtuStress -width 10
label .exportUI.frmAllowStress.lbl2 -text "Fcy" 
entry .exportUI.frmAllowStress.entFcy -textvariable ::FcyStress -width 10
label .exportUI.frmAllowStress.lbl3 -text "Fsu" 
entry .exportUI.frmAllowStress.entFsu -textvariable ::FsuStress -width 10
pack .exportUI.frmAllowStress.lbl .exportUI.frmAllowStress.entFtu \
     .exportUI.frmAllowStress.lbl2 .exportUI.frmAllowStress.entFcy \
     .exportUI.frmAllowStress.lbl3 .exportUI.frmAllowStress.entFsu \
     -side left -padx 5
...
button .exportUI.btnStress -text "Stress VonMises, Max/Min Prin, Max Shear" -command ExportStressForSet
...
pack .exportUI.frmAllow.lblFtu .exportUI.frmAllow.entFtu \
     .exportUI.frmAllow.lblFcu .exportUI.frmAllow.entFcu \
     .exportUI.frmAllow.lblFsu .exportUI.frmAllow.entFsu \
     -side left -padx 5
...
proc GetExtreme {maxval minval} {
    if {[expr {abs($minval)}] > [expr {abs($maxval)}]} {
        return [FormatNumber $minval 2]
    } else {
        return [FormatNumber $maxval 2]
    }
}

proc FormatNumber {value {decimals 3}} {
    if {$value == "" || ![string is double -strict $value]} {
        return "N/A"
    }
...
proc ExportDisplacement {} {
    ...
    catch {cont SetDataType "Displacement"}
    ...
    foreach sub $subcase_list {
        ...
        set mag_max [FormatNumber [leg GetMax] 2]
        leg ReleaseHandle
...
proc ExportStressForSet {} {
    global user_elem_sel FtuStress FcyStress FsuStress
    ...
    puts $fp "Loadcase_ID;Loadcase_Name;Max_VonMises;RF_VonMises;Max_MaxPrincipal;RF_MaxPrincipal;Min_MinPrincipal;RF_MinPrincipal;MaxShear;RF_MaxShear"
    ...
    foreach sub $subcase_list {
        ...
        if {$num_elems > 0} {
            ...
            # MAX SHEAR (Layer = Max)
            cont SetEnableState false
            catch {cont SetDataComponent $c_shear}
            catch {cont SetShellLayer "Max"}
            cont SetEnableState true
            cli Draw
            qry GetIteratorHandle iter
            for {iter First} {[iter Valid]} {iter Next} {
                set val [lindex [iter GetDataList] 1]
                if {[string is double -strict $val]} {
                    if {$max_shear == "" || $val > $max_shear} { set max_shear $val }
                }
            }
            iter ReleaseHandle
        }
        ...
        if {$max_shear != ""} { if {$g_max_shear == "" || $max_shear > $g_max_shear} { set g_max_shear $max_shear } }
        
        # Calculate RF values
        set rf_vonMises [SafeRF $FtuStress $max_von]
        set rf_maxPrin [SafeRF $FtuStress $max_p1]
        set rf_minPrin [SafeRF $FtuStress $min_p3]
        set rf_maxShear [SafeRF $FsuStress $max_shear]
        
        # Format for CSV
        set str_von [expr {$max_von != "" ? "[FormatNumber $max_von 2]" : "N/A"}]
        set str_p1 [expr {$max_p1 != "" ? "[FormatNumber $max_p1 2]" : "N/A"}]
        set str_p3 [expr {$min_p3 != "" ? "[FormatNumber $min_p3 2]" : "N/A"}]
        set str_shear [expr {$max_shear != "" ? "[FormatNumber $max_shear 2]" : "N/A"}]
        
        puts $fp "$sub;$safe_sub_name;$str_von;$rf_vonMises;$str_p1;$rf_maxPrin;$str_p3;$rf_minPrin;$str_shear;$rf_maxShear"
    }
...
proc ExportFailureIndexForSet {} {
    ...
    foreach sub $subcase_list {
        ...
        if {$max_fi != ""} { 
            set str_fi [FormatNumber $max_fi 4] 
            if {$max_fi > 0} {
                set str_rf [FormatNumber [expr {1.0 / sqrt($max_fi)}] 3]
            } else {
                set str_rf "N/A"
            }
        } else { 
...
proc ExportInterLaminarStressForSet {} {
    ...
    foreach sub $subcase_list {
        ...
        # Format for CSV
        if {$max_yz != ""} { set str_max_yz [FormatNumber $max_yz 3] } else { set str_max_yz "N/A" }
        if {$min_yz != ""} { set str_min_yz [FormatNumber $min_yz 3] } else { set str_min_yz "N/A" }
        if {$max_xz != ""} { set str_max_xz [FormatNumber $max_xz 3] } else { set str_max_xz "N/A" }
        if {$min_xz != ""} { set str_min_xz [FormatNumber $min_xz 3] } else { set str_min_xz "N/A" }
...
proc ExportStressExtremeLayer {} {
    ...
    foreach sub $subcase_list {
        ...
        for {set i 0} {$i < 6} {incr i} {
            ...
            set str_max [expr {$c_max != "" ? [FormatNumber $c_max 3] : "N/A"}]
            set str_min [expr {$c_min != "" ? [FormatNumber $c_min 3] : "N/A"}]
        ...
        set str_max_out [string trimright $str_max_out ";"]
        set str_min_out [string trimright $str_min_out ";"]
    ...
    set g_str_max_xx [expr {$g_max_xx != "" ? [FormatNumber $g_max_xx 3] : "N/A"}]
    set g_str_min_xx [expr {$g_min_xx != "" ? [FormatNumber $g_min_xx 3] : "N/A"}]
    set g_str_max_yy [expr {$g_max_yy != "" ? [FormatNumber $g_max_yy 3] : "N/A"}]
    set g_str_min_yy [expr {$g_min_yy != "" ? [FormatNumber $g_min_yy 3] : "N/A"}]
    set g_str_max_zz [expr {$g_max_zz != "" ? [FormatNumber $g_max_zz 3] : "N/A"}]
    set g_str_min_zz [expr {$g_min_zz != "" ? [FormatNumber $g_min_zz 3] : "N/A"}]
    set g_str_max_xy [expr {$g_max_xy != "" ? [FormatNumber $g_max_xy 3] : "N/A"}]
    set g_str_min_xy [expr {$g_min_xy != "" ? [FormatNumber $g_min_xy 3] : "N/A"}]
    set g_str_max_zx [expr {$g_max_zx != "" ? [FormatNumber $g_max_zx 3] : "N/A"}]
    set g_str_min_zx [expr {$g_min_zx != "" ? [FormatNumber $g_min_zx 3] : "N/A"}]
    set g_str_max_yz [expr {$g_max_yz != "" ? [FormatNumber $g_max_yz 3] : "N/A"}]
    set g_str_min_yz [expr {$g_min_yz != "" ? [FormatNumber $g_min_yz 3] : "N/A"}]
...
proc ExportStressExtremeLayerZXYZ {} {
    ...
    # OUTPUT COLUMN ORDER (Fixed commas to semicolons)
    ...
    foreach sub $subcase_list {
        ...
        for {set i 0} {$i < 2} {incr i} {
            ...
            # Local string format (Using FormatNumber for French comma)
            set str_max [expr {$c_max != "" ? [FormatNumber $c_max 3] : "N/A"}]
            set str_min [expr {$c_min != "" ? [FormatNumber $c_min 3] : "N/A"}]
            ...
            # Append maxes to one string, mins to another (Properly formatted)
            append str_max_out ";$str_max;$rf_max"
            append str_min_out ";$str_min;$rf_min"
        }

        # Removed the string trimright since append is clean now
        puts $fp "$sub;$safe_sub_name$str_max_out$str_min_out"
    }
